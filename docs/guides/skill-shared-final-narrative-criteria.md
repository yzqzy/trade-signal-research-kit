...
仅当 **「终稿呈现」+ §1～§4 + [PDF 门禁语义](./skill-shared-pdf-gate-semantics.md) + 叙事质量门禁** 全部满足时，方可使用 **`[终稿状态: 完成]`**。否则文件末尾须为 **`[终稿状态: 阻断]`**，并在对话中说明阻断原因。发布层会重新校验；模板稿即使写了 `[终稿状态: 完成]`，也不会进入文档站。

### 6) D1 商业模式与数值证据（与 workflow `business_quality.md` 门禁对齐）

`qualitative_d1_d6.md` 的 **`## D1`**（或等价标题）除 §1 证据底线外还须满足：

- **可核验数字**：正文 D1 段至少 **8 个**数字字符（金额、比率、%、年度等），且能回落到 `data_pack_market.md` / `data_pack_report.md` 或附录表（禁止「证据基础已建立」式空话）。
- **商业模式判定**：须写清 **主赚钱机制 + 当前约束**（可与 `report_view_model.json` 的 `businessModel` 叙事一致，但终稿仍以证据表述为准）。
- **监控表**：至少 **3 行数据**写清 **可执行门槛**（如 `<5%`、`>1.2×`、`连续 2 年` 等与数字结合的阈值），禁止仅写「方向性跟踪」而无门槛（与 TS 门禁对「监控阈值」小节解析一致）。
- **降级声明**：若市场包 §13 命中 **多年外推复制/关键估算规则**，或 `gateVerdict` 为 **CRITICAL**，D1 必须显式说明 **结论降级 / 证据边界**（与 TS 门禁 `evaluateBusinessQualityPublicationHardBlock` 语义一致）。

### 7) 叙事质量门禁（模板稿不得发布）

终稿必须接近 `references/analysis` 的研报表达密度：先给 verdict / Executive Summary / Quality Snapshot / 核心发现，再展开 D1-D6。每个维度必须包含 **自然语言判断 + 行业特定变量 + 关键数字 + 反证条件 + 证据引用**。以下情况视为模板稿，不得标完成：

- 裸露工程枚举：`middle`、`high`、`medium`、`weak`、`unknown` 等直接出现在正文判断中，而非翻译为自然语言并解释投资含义。
- 流程说明替代判断：如“商业质量终稿基于年报包、市场包和外部证据完成”。
- 泛化模板句替代分析：如“不能只看单期利润或榜单排名”“当前仅按证据包建立方向性跟踪”“验证本维判断是否持续”。
- D2-D6 大面积复用同一“证据链条 / 结论 / 可跟踪指标”段落。
- 只有表格和指标，没有解释为什么这些数字改变商业质量判断。
...
- [ ] **D1** 缺少数值证据（正文 D1 段可核验数字**少于 8**）、监控表无 **≥3 行**带阈值、或缺商业模式/约束表述
- [ ] **D1** 在命中外推/关键估算或 **CRITICAL** 时未写 **降级与证据边界**
