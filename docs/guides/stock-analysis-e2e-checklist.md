...
本清单用于验证：从单只 A 股输入到研究报告输出的完整链路可跑通。

## 目标

- 验证 CLI 主入口（`phase0` / `business-analysis` / `workflow` / `valuation` / `report-to-html`）可联通。
- 验证财务新路径（`financial/snapshot`、`financial/history`）已在 provider 中生效。
- 验证质量门禁（`quality:all`）通过后，链路具备可回归性。
...
```bash
pnpm run phase0:download -- --code 600887 --year 2024
```
...
- [ ] 验收产物（`output/workflow/600887/<runId>/`）：
  - [ ] `analysis_report.md`
  - [ ] `analysis_report.html`（或可由 `report-to-html` 生成）
...
以下字段目前允许 `undefined`，由 `degradeReasons` 说明：
