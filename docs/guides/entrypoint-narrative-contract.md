...
| `/financial-minesweeper` / `financial-minesweeper:run` | HTTP Feed 多年财报与质量趋势 + **强制当年 PDF**（自动发现下载，失败即终止；可用 `--report-url` 兜底），规则化「财报排雷」报告 + manifest；可进站为 **`financial-minesweeper`** 专题 | **否**（确定性 TS；非六维终稿路径） |
