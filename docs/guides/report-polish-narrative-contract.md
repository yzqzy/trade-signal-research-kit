...
本仓库在 **Phase3（`stageE`）之后** 增加 **`reportPolish`** 阶段：生成 `report_view_model.json` 与四份 Markdown（`turtle_overview.md`、`business_quality.md`、`penetration_return.md`、`valuation.md`），并供 `reports-site:emit` 优先作为各专题 `content.md` 来源。
...
3. **引用边界**：商业质量页以 `phase1b_qualitative.md` + Phase3「因子1B」为主；**不等价**于 `business-analysis:run` 产出的 `qualitative_report.md` 六维终稿（页面内已声明）。
...
- `reports-site:emit --run-dir …` 在 manifest 含 polish 路径时，**优先**采用 polish 文件作为专题正文来源。
