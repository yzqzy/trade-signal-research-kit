...
- A 股单期合并快照：`FeedHttpProvider.getFinancialSnapshot` 优先 `GET /api/v1/stock/financial/snapshot/:code?reportDate=`（`reportDate` 须为年报期末 `YYYY-12-31`），失败回退 `GET /stock/indicator/financial/:code`（简表首条）。
- A 股多年财报：`FeedHttpProvider.getFinancialHistory` 优先 `GET /api/v1/stock/financial/history/:code`，失败回退按年 `getFinancialSnapshot`。
- 全部查询统一走 HTTP 通道
...
- AI 场景：同样使用 `HTTP`（Agent 交互、工具链编排、动态检索补充）
...
- 同一输入参数在 HTTP 产出同构语义（字段含义、单位、时区一致）
...
**`workflow:run` 编排**：固定使用 HTTP `createFeedHttpProviderFromEnv()` 采集 Phase1A，需配置 `FEED_BASE_URL`。

## Phase1B（外部信息补全）调用方式

- 聚合入口：`collectPhase1BQualitative(input, options)`（默认 `channel=http`）
- 渲染入口：`renderPhase1BMarkdown(supplement)`
- 编排产物（`business-analysis` / workflow 主链）：同目录写入 `phase1b_evidence_quality.json`（§7/§8 唯一标题占比、跨条目 URL 重复、主题命中率、§7§8 URL Jaccard），用于离线对比两次 run 质量。
...
- 设置 `input.channel = "http"`
- 统一通过 Feed HTTP API 检索
...
- **conformance**：`pnpm run quality:conformance`  
  - 以同源 fixture 驱动已构建的 `provider-http`，校验 `instrument/quote/klines/financial/corporateActions/tradingCalendar` 的语义一致性（不访问真实 Feed）。
