...
### 与 `workflow` / report-polish 的边界（必读）

- **`/workflow-analysis`（`workflow:run`）**：产出规则报告 `analysis_report.md`、估值 JSON，以及 **report-polish** 四页 Markdown + `report_view_model.json`；经 `reports-site:emit` 进入研报站。**本 skill 不负责**改写上述 workflow 发布页。
- **`/business-analysis`（本 skill）**：专责 **六维定性终稿** 写回 `qualitative_report.md` / `qualitative_d1_d6.md`；与 report-polish 的「多页发布稿」**并行存在、职责不重叠**。若同一标的既要站点多页又要六维终稿，分别跑两条入口并各自验收。
