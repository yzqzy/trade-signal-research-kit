...
- **文末必须**有 **`## 附录：证据索引`**（Markdown 表格），列：`证据ID | 类型（PDF/公告/市场包） | 摘要 | 链接或定位（PDF 写 `p.N`；市场包写文件内章节）`。同一 run 内两份终稿 **优先共用一套 `E` 编号**。
- **软约束**：完成硬约束后，可选 **`## 终稿自检（软）`**（1～5 分），不阻断。

## PDF 与「终稿完成」语义（默认尝试 + 绑定阻断）

- **编排层（CLI）**：无 `--pdf`/`--report-url` 时已 **best-effort** Phase0 拉取年报并跑 Phase2B（见 command）；收口前仍须在会话内打开 run 目录 **`data_pack_report.md`**（若存在）核对 **`gateVerdict`** / 低置信提示。
- **不得标 `[终稿状态: 完成]`** 的情形（须 **`[终稿状态: 阻断]`** 并写清原因）：
  ...
  2. `gateVerdict` 为 **`CRITICAL`**。
  3. `gateVerdict` 为 **`DEGRADED`**（或正文存在 Phase2B 低置信/缺失警告）：须 **`> PDF 抽取质量声明`**，**不得**宣称终稿证据已充分核验。
- 上述情形下仍可按硬约束写叙事草稿，但 **只对用户声明「已阻断写回」**，不宣称「终稿完成」。
...
- 若 `data_pack_report.md` 含 **`gateVerdict`** 为 **`DEGRADED`** / **`CRITICAL`**，或正文有 Phase2B 低置信/缺失警告：在 `qualitative_report.md` 与 `qualitative_d1_d6.md` 中均须增加简短 **`> PDF 抽取质量声明`**，说明依赖人工复核、不得将细节当作高置信事实。

### 4) 「终稿完成」判定（收紧）

仅当 **§0（呈现）+ §1～§3 + 上文「PDF 与终稿完成语义」** 全部满足时，方可使用 **`[终稿状态: 完成]`**。否则文件末尾须为 **`[终稿状态: 阻断]`**，并在对话中说明阻断原因。
...
- [ ] 无 `data_pack_report.md` / `gateVerdict` 为 CRITICAL 或 DEGRADED，但未按「PDF 与终稿完成语义」写声明
