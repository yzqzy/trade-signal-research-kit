...
| P1A-HISTORY | Phase1A 拉取多年财报（`getFinancialHistory` 优先 `feed` `/stock/financial/history`，失败按年 `getFinancialSnapshot`；`getFinancialSnapshot` 优先 `/stock/financial/snapshot`） | `phase1a/financial-history.ts`、`provider-http`、`trade-signal-feed` | implemented |
