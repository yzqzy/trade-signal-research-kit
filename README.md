...
| 模块 | Slash（推荐） | 关键产物 |
|------|----------------|----------|
| 全流程研究 | `/workflow-analysis` | `analysis_report.md`、`valuation_computed.json`、`workflow_manifest.json`、report-polish 四页 + `report_view_model.json`（六维终稿见 `/business-analysis`） |
...
一次跑完 **Phase0（可选）→ Phase1A →（有年报 PDF 时）Phase2A/2B → Phase1B → Phase3**。先看 `analysis_report.md`、`valuation_computed.json`、`workflow_manifest.json`。
...
|----------|----------------|----------|--------|
| 一次跑完采集 →（可选）年报 → 外部补充 → 策略/估值 + 发布稿整形 | `/workflow-analysis` | 上表 + report-polish 多页 Markdown | 研报站 `reports-site:emit` → `sync:reports-to-app` |
...
| 研报中心（静态索引） | — | `output/site/reports/**` → `apps/research-hub/public/reports`（`content.md` v2） | `pnpm run reports-site:emit` → `pnpm run sync:reports-to-app` → 见 [reports-site-publish](docs/guides/reports-site-publish.md) |
...
| 类别 | 说明 |
|------|------|
| **主要能力** | 全流程 `/workflow-analysis`、商业分析 `/business-analysis`、独立估值、独立 Phase3（包内 `run:phase3`）、年报获取、选股器、研报站 Markdown 发布 |
