...
> **命名说明**：本仓库根 `package.json` 名为 `trade-signal-research-kit`；核心可执行/编排与 CLI 入口为 **`@trade-signal/research-runtime`**。领域五对象与插件分层另见 `packages/research-contracts` 与 `packages/research-feature|policy|topic|selection`。
