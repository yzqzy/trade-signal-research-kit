...
| 模块 | Slash（推荐） | 关键产物 |
|------|----------------|----------|
| 全流程研究 | `/workflow-analysis` | `analysis_report.md`、`valuation_computed.json`、`workflow_manifest.json`（定性深度补强见契约文档） |
...
|----------|----------------|----------|--------|
| 一次跑完采集 →（可选）年报 → 外部补充 → 策略/估值/终稿 | `/workflow-analysis` | `analysis_report.md`、`valuation_computed.json`、`workflow_manifest.json` | 研报站见 `reports-site:emit` |
